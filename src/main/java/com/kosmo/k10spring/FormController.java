...
/*
 ...
 패키지로 지정된 com.kosmo.k10spring을 스캔하여 컨트롤러 클래스를 찾는다.
 ...
 @RequestMapping
 	: 요청명을 매핑한다. 요청명은 서블릿과 마찬가지로 컨택스트 루트를 제외한 나머지 경로명으로 이루어진다.
...
public class FormController {
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		/*
		 View의 페이지경로를 반환한다. 아래처럼 String으로 변환하면 ViewResolver가 전체경로를 조립하여 웹브라우저에 출력한다.
		 */
	...
	/*
	 ...
	 조건1 : 쿼리스트링으로 전달되는 파라미터의 갯수와 폼값을 저장할 객체(DTO 혹은 VO)의 멤버변수의 갯수가 동일할 때 사용할 수 있다.
	 조건2 : 커맨드 객체를 사용할때는 클래스명 앞글자를 소문자로 바꾼 형태의 매개변수를 사용해야 한다.
	 	Ex) MemberDTO -> memberDTO 
	 조건3 : 커맨드객체를 만들 때 파라미터의 갯수는 달라도 상관없으나 파라미터명과 멤버변수명은 동일해야하고 
	...
	/*
	 ...
	 파라미터를 마치 경로명처럼 사용하게 되고, 이때 주의해야 할점은 
	 웹브라우저는 요청명을 경로로 인식하므로 리소스를 사용할 때 경로를 주의해야 한다.
